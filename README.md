...
- OpenAI Chat Completions 风格接口和 Responses 接口适配；
...
打开 `http://localhost:8000`，点击右上角“本地书库”：

1. 填写模型 Base URL、模型 ID 和可选 API Key；
2. 保存并测试连接；
3. 先将一本书加入“我的书架”（实体书、阅读 App 中读过的书也可以）；
...
API Key 不会通过设置 API 回显。网页保存的密钥位于本地 `data/bookmate.db`；如果电脑有多人账户或数据目录会被同步，优先使用环境变量或系统秘密管理工具。
